...
BEGIN
    ...
    SET @XMLData = (
        ...
        INNER JOIN Hospital.Especialidad AS E ON T.id_especialidad = ME.id_especialidad
        INNER JOIN Turno.EstadoTurno AS ET ON T.id_estado_turno = ET.id_estado
        INNER JOIN ObraSocial.Prestador AS OS ON CO.id_prestador = OS.id_prestador -- Nacho: me falta traer el prestador (hacer el join)
...
IF NOT EXISTS (SELECT
        ...
        INNER JOIN Paciente.Paciente AS P ON T.id_historia_clinica = P.id_historia_clinica
        INNER JOIN Hospital.Medico AS PR ON T.id_medico = PR.id_medico
        INNER JOIN Hospital.MedicoEspecialidad AS ME ON T.ID_Especialidad = ME.ID_Especialidad
        INNER JOIN Turno.EstadoTurno AS ET ON T.id_estado_turno = ET.id_estado
        INNER JOIN ObraSocial.Prestador AS OS ON CO.id_prestador = OS.id_prestador)
	begin
		exec Paciente.InsertarCobertura 1,Null,NULL --Inserto cobertura con id prestador 1
		update Paciente.Paciente set paciente.ID_Cobertura=1 where Paciente.ID_HistoriaClinica=1  --actualizo paciente seteando cobertura=1
